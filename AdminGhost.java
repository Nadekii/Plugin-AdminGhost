...
public class AdminGhost extends JavaPlugin {

	public static final Logger log = Logger.getLogger("Minecraft");

	Permissions CurrentPermissions = null;
	MySQLDatabase db;
	String maindir = "plugins/AdminGhost/";
	...
	public void onDisable() {
		tempBans.clear();
		bannedPlayers.clear();
		System.out.println("AdminGhost disabled.");
	}
	...
	private boolean unBanPlayer(CommandSender sender, String[] args){
		...
		if (sender instanceof Player){
			...
			if (Permissions.Security.permission(player, "gadmin.unban")) auth=true;
		...
		if(bannedPlayers.remove(p.toLowerCase())){
			...
			//Log in console
			log.log(Level.INFO, "[AdminGhost] " + kicker + " unbanned player " + p + ".");
	...
	private boolean kickPlayer(CommandSender sender, String[] args){
		...
		if (sender instanceof Player){
			player = (Player)sender;
			if (Permissions.Security.permission(player, "gadmin.kick")) auth=true;
			kicker = player.getName();
		}else{
			auth = true;
		}
		...
		if(p.equals("*")){
			if (sender instanceof Player)
				if (!Permissions.Security.permission(player, "gadmin.kick.all")) return false;
			...
			log.log(Level.INFO, "[AdminGhost] " + formatMessage(kickerMsg));
		...
		//Log in console
		log.log(Level.INFO, "[AdminGhost] " + kicker + " kicked player " + p + ". Reason: " + reason);
	...
	private boolean banPlayer(CommandSender sender, String[] args){
		...
		if (sender instanceof Player){
			player = (Player)sender;
			if (Permissions.Security.permission(player, "gadmin.ban")) auth=true;
			kicker = player.getName();
		}else{
			auth = true;
		}
		...
		//Log in console
		log.log(Level.INFO, "[AdminGhost] " + kicker + " banned player " + p + ".");
	...
	private boolean tempbanPlayer(CommandSender sender, String[] args){
		...
		if (sender instanceof Player){
			player = (Player)sender;
			if (Permissions.Security.permission(player, "gadmin.tempban")) auth=true;
			kicker = player.getName();
		}else{
			auth = true;
		}
		...
		//Log in console
		log.log(Level.INFO, "[AdminGhost] " + kicker + " tempbanned player " + p + ".");
	...
	private boolean ipBan(CommandSender sender, String[] args){
		...
		if (sender instanceof Player){
			player = (Player)sender;
			if (Permissions.Security.permission(player, "gadmin.ipban")) auth=true;
			kicker = player.getName();
		}else{
			auth = true;
		}
	...
	private boolean warnPlayer(CommandSender sender, String[] args){
		...
		if (sender instanceof Player){
			player = (Player)sender;
			if (Permissions.Security.permission(player, "gadmin.warn")) auth=true;
			kicker = player.getName();
		}else{
			auth = true;
		}
		...
		//Log in console
		log.log(Level.INFO, "[AdminGhost] " + kicker + " warned player " + p + ".");
	...
	private boolean reloadKA(CommandSender sender){
		...
		if (sender instanceof Player){
			player = (Player)sender;
			if (Permissions.Security.permission(player, "gadmin.reload")) 
				auth=true;
			kicker = player.getName();
		}else{
			auth = true;
		...
		if (auth) {
			...
			log.log(Level.INFO, "[AdminGhost] " + kicker + " reloaded the banlist.");
	...
	private boolean exportBans(CommandSender sender){
		...
		if (sender instanceof Player){
			player = (Player)sender;
			if (Permissions.Security.permission(player, "gadmin.export")) 
				auth=true;
		}else{
			auth = true;
		}
		if (auth) {
			...
			{
				KiwiAdmin.log.log(Level.SEVERE,"AdminGhost: Couldn't write to banned-players.txt");
			}
